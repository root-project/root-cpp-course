...
void read(std::string_view dataset_name, std::string_view file_name,
  ...
  // Reading API: RNTupleReader::GetView. Creates an RNTupleView object which
  // can be used to access values of the corresponding field
  auto view_muon_pt = reader->GetView<std::vector<float>>("muon_pt");
  for (auto entry_idx : *reader) {
    ...
    const auto &muon_pt = view_muon_pt(entry_idx);
    std::cout << "muon_pt={";
...
void example_04_rntuple_metrics() {
  constexpr auto file_name{"example_04_rntuple_metrics.root"};
