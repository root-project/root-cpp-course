...
void exercise_03_higgs_to_four_leptons_solution() {
  ...
  // Define metadata in the computation graph
  auto df_with_metadata =
      df.DefinePerSample("xsecs",
                         [](unsigned int, const RSampleInfo &id) {
                           return id.GetD("xsecs");
                         })
          .DefinePerSample("lumi",
                           [](unsigned int, const RSampleInfo &id) {
                             return id.GetD("lumi");
                           })
          .DefinePerSample("sumws",
                           [](unsigned int, const RSampleInfo &id) {
                             return id.GetD("sumws");
                           })
          .DefinePerSample("sample_category",
                           [](unsigned int, const RSampleInfo &id) {
                             return id.GetS("sample_category");
                           })
          .DefinePerSample("scale",
                           [](unsigned int, const RSampleInfo &id) {
                             return id.Contains("mc_363490.llll.4lep.root")
                                        ? 1.3f
                                        : 1.0f;
                           })
          .DefinePerSample("reweighting",
                           [](unsigned int, const RSampleInfo &id) {
                             return id.Contains("mc");
                           });
  ...
  auto df_mass =
      df_filtered
          .Define("goodlep_pt",
                  [](const RVecF &v, const RVecI &m) { return v[m]; },
                  {"lep_pt", "good_lep"})
          .Define("goodlep_eta",
                  [](const RVecF &v, const RVecI &m) { return v[m]; },
                  {"lep_eta", "good_lep"})
          ...
          .Define("goodlep_E",
                  ...
                  {"lep_E", "good_lep"})
          .Define("goodlep_type",
                  [](const RVecU &v, const RVecI &m) { return v[m]; },
                  {"lep_type", "good_lep"})
          .Filter(
              [](const RVecF &pt) {
                return pt[0] > 25000.f && pt[1] > 15000.f && pt[2] > 10000.f;
              },
              {"goodlep_pt"})
          .Define("m4l", ComputeInvariantMass,
                  {"goodlep_pt", "goodlep_eta", "goodlep_phi", "goodlep_E"});
  ...
  auto df_higgs = df_mc.Filter(categoryIs("higgs"), {"sample_category"})
                      .Histo1D<float, float>(
                          ROOT::RDF::TH1DModel("higgs", "m4l", 24, 80, 170),
                          "m4l", "weight");
  auto df_zz =
      df_mc.Filter(categoryIs("zz"), {"sample_category"})
          .Histo1D<float, float>(ROOT::RDF::TH1DModel("zz", "m4l", 24, 80, 170),
                                 "m4l", "weight");
  auto df_other = df_mc.Filter(categoryIs("other"), {"sample_category"})
                      .Histo1D<float, float>(
                          ROOT::RDF::TH1DModel("other", "m4l", 24, 80, 170),
                          "m4l", "weight");

  // Histogram of data
  auto df_h_mass_data =
      df_mass
          .Filter([](bool reweighting) { return !reweighting; },
                  {"reweighting"})
          .Filter(categoryIs("data"), {"sample_category"})
          .Define("weight", [] { return 1.0f; })
          .Histo1D<float, float>(
              ROOT::RDF::TH1DModel("data", "m4l", 24, 80, 170), "m4l",
              "weight");
